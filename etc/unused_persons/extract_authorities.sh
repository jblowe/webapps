...
eval `grep CREDS  authextract.cfg`
eval `grep SERVER authextract.cfg`
...
HERE
   ...
   while [ $MAXCURLS -gt 0 ]; do
       ...
       if ! curl -S -s -u "${CREDS}" -o /tmp/tmp.xml "${SERVER}/cspace-services/$1?pgSz=${PAGESIZE}&pgNum=${PAGENUM}&wf_deleted=false"
       then
           echo ERROR: cURL failed.
           echo "curl -S -s -u \"CREDS\" -o /tmp/tmp.xml \"${SERVER}/cspace-services/$1?pgSz=${PAGESIZE}&pgNum=${PAGENUM}&wf_deleted=false\""
           cat /tmp/tmp.xml
           echo
           exit 1
        fi
