...
def getrefname(csid, vocab):
    info = requests.get(baseurl + '/' + vocab + 'authorities/urn:cspace:name('+vocab+')/items/' + csid,
...
def getobjs(oldcsid):
    usedby = requests.get(baseurl + '/' + termvocab + 'authorities/urn:cspace:name(' + termvocab + ')/items/' + oldcsid +
