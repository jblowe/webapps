...
gunzip -fk 4solr.omca.media-public.csv.gz
cut -f8,16,17 4solr.omca.media-public.csv | sort | uniq | grep -v blobcsid | grep -v ' rows' > s3_current_blobs.csv
...
# tidy up
rm s3_c.tmp 4solr.omca.media-public.csv
gzip -f s3_*.csv
