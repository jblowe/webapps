...
SELECT DISTINCT cc.id AS id, STRING_AGG(regexp_replace(regexp_replace(grp.hazard, '^.*\)''(.*)''$', '\1'), E'[\\t\\n\\r]+', ' ', 'g'), '␥') AS hazard_ss
FROM conditionchecks_common lc
JOIN misc m on (m.id = lc.id AND m.lifecyclestate <> 'deleted')
JOIN hierarchy h ON (h.parentid=lc.id AND h.primarytype = 'hazardGroup')
JOIN hazardgroup grp ON (grp.id=h.id)
