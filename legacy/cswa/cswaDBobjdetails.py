...
def getparentinfo(museumNumber, config):
    omcadb = psycopg2.connect(config.get('connect', 'connect_string'))
    objects = omcadb.cursor()
   ...
   SELECT c1.objectnumber, 
    ...
    cop.omcatmslegacydepartment,
    cop.omcafieldlocverbatim,
...
JOIN collectionobjects_omca cp1 ON (c1.id = cp1.id)
...
JOIN collectionobjects_omca cp2 ON (c2.id = cp2.id)
...
LEFT OUTER JOIN collectionobjects_common_contentconcepts efc ON (c2.id=efc.id AND efc.pos=0)
JOIN collectionobjects_omca cop ON (c2.id=cop.id)
LEFT OUTER JOIN collectionobjects_common_contentplaces fcp ON (c2.id=fcp.id AND fcp.pos=0)
...
def getobjinfo(museumNumber, config):
   ...
   SELECT co.objectnumber,
    ...
    CASE WHEN (efc.item IS NOT NULL AND efc.item <> '') THEN SUBSTRING(efc.item, POSITION(')''' IN efc.item)+2, LENGTH(efc.item)-POSITION(')''' IN efc.item)-2) END AS filecode,
    cp.iscomponent,
    cp.omcatmslegacydepartment,
    cp.omcafieldlocverbatim,
    CASE WHEN (fcp.item IS NOT NULL AND fcp.item <> '') THEN SUBSTRING(fcp.item, POSITION(')''' IN fcp.item)+2, LENGTH(fcp.item)-POSITION(')''' IN fcp.item)-2) END AS site,
    cm.item,
    csid.name,
    co.id,
...
LEFT OUTER JOIN collectionobjects_common_contentconcepts efc ON (co.id=efc.id AND efc.pos=0)
JOIN collectionobjects_omca cp ON (co.id=cp.id)
...
def getaltnums(objectid, config):
    ...
    getaltnums = """
   SELECT an.numbervalue,
    an.numbertype,
    an.numbervaluenote
FROM hierarchy h1
JOIN othernumber an ON (h1.id=an.id AND h1.pos=0)
WHERE h1.name='collectionobjects_omca:omcaAltNumGroupList'
...
def getallaltnums(museumNumber, config):
    ...
    getallaltnums = """
   SELECT co.objectnumber,
    an.numbervalue AS altnum,
    an.numbertype AS altnumtype,
    an.numbervaluenote AS altnumnote
FROM collectionobjects_common co
LEFT OUTER JOIN hierarchy h1 ON (co.id = h1.parentid AND h1.name='collectionobjects_omca:omcaAltNumGroupList')
JOIN othernumber an ON (h1.id=an.id AND h1.pos=0)
WHERE co.objectnumber = '%s'""" % museumNumber
...
JOIN media_omca mp ON (mc.id = mp.id)
